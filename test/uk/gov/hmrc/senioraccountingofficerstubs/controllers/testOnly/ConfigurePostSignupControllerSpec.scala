...
import uk.gov.hmrc.senioraccountingofficerstubs.controllers.testOnly.ConfigurePostSignupControllerSpec.*
...
class ConfigurePostSignupControllerSpec
    extends AnyWordSpec
    with Matchers
  ...
  private def fakeRequest(payload: String) =
    FakeRequest("POST", "/test-only/post-signup/config")
      .withBody(payload)
  ...
  "ConfigurePostSignupController" must {
    ...
    "return 200 for a valid request and persist it into Mongo" in {
      val expectedConfig = PostSignupStubConfiguration(
        subscriptionId = testSubscriptionId,
        getSubscriptionAndPostRetrieveCustomerId = Some(GetSubscriptionOnlyConfig(status = 500)),
        postNotification = Some(NoneDefaultApiConfiguration(status = 400)),
        postCertificate = Some(NoneDefaultApiConfiguration(status = 404)),
        lastUpdated = instant
      )
      ...
      verify(mockRepository, times(1)).set(
        argThat((capturedConfig: PostSignupStubConfiguration) =>
...
object ConfigurePostSignupControllerSpec {
